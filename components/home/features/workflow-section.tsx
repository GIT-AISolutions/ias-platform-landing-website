...
export function WorkflowSection() {
  return (
    <section
      id="product"
      className="border-y border-[var(--nexra-border)]/90 bg-[color-mix(in_srgb,var(--nexra-bg-panel)_78%,transparent)]"
    >
      <div className="nexra-section">
        <div className="nexra-section-heading nexra-reveal">
          <p className="text-sm uppercase tracking-[0.16em] text-[var(--nexra-accent)]">
            How It Works
          </p>
          <h2 className="text-3xl md:text-5xl">How Nexra works</h2>
        ...
        <div className="relative mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          <div className="pointer-events-none absolute left-[10%] right-[10%] top-10 hidden h-px bg-gradient-to-r from-transparent via-[var(--nexra-accent)]/60 to-transparent xl:block" />
          {workflow.map((item, index) => (
            <Panel
              key={item.step}
              className={cn(
                "nexra-interactive-card nexra-reveal relative flex h-full flex-col gap-3",
                index % 5 === 1 ? "nexra-reveal-delay-1" : "",
                index % 5 === 2 ? "nexra-reveal-delay-2" : "",
                index % 5 === 3 ? "nexra-reveal-delay-3" : "",
              )}
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium uppercase tracking-[0.14em] text-[var(--nexra-accent)]">
                  Step {item.step}
                </span>
                <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-[var(--nexra-border)] bg-[var(--nexra-bg-elevated)] text-[var(--nexra-accent)]">
        ...
        <Panel className="mt-6 p-0 nexra-reveal nexra-reveal-delay-2">
          <div className="grid gap-0 md:grid-cols-3">
            {[
              ...
              <div
                key={label}
                className="border-b border-[var(--nexra-border)] p-5 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0"
              >
