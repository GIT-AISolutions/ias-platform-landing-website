...
try {
  ...
  for (const viewport of VIEWPORTS) {
    ...
    const stickySamples = [];
    for (const progress of [0.2, 0.62, 0.85]) {
      await page.evaluate((p) => {
        const section = document.querySelector('#video-demo');
        const top = section.getBoundingClientRect().top + window.scrollY;
        const end = top + section.offsetHeight - window.innerHeight;
        window.scrollTo(0, top + (end - top) * p);
      }, progress);
      await page.waitForTimeout(450);
      stickySamples.push(await page.evaluate((p) => {
        const sticky = document.querySelector('.vd-sticky');
        return {
          progress: p,
    ...
    assert.ok(Math.abs(first.stickyTop) <= 2, `${viewport.name}: video demo panel should stay sticky while scrolling; top=${first.stickyTop}`);
    for (const sample of stickySamples) {
      assert.ok(Math.abs(sample.top) <= 2, `${viewport.name}: video demo should remain pinned at progress ${sample.progress}; top=${sample.top}`);
      assert.ok(Math.abs(sample.bottom - viewport.height) <= 2, `${viewport.name}: pinned panel should fill the viewport at progress ${sample.progress}; bottom=${sample.bottom}`);
    }
    assert.ok(first.horizontalOverflow <= 2, `${viewport.name}: page should not create horizontal overflow`);
