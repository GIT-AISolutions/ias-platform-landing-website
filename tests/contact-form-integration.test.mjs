...
assert.match(stylesCss, /\.site-chat-log\s*{[\s\S]*gap: 0;[\s\S]*overflow-y: auto;[\s\S]*overflow-x: hidden;[\s\S]*padding: 1rem 1\.375rem 1\.125rem 1rem;/, 'chat log should scroll vertically with breathing room before the scrollbar');
assert.match(stylesCss, /\.site-chat-msg\s*{[\s\S]*display: flex;[\s\S]*flex: 0 0 auto;[\s\S]*width: 100%;[\s\S]*margin: 0 0 0\.875rem;[\s\S]*background: transparent;/, 'message wrappers should reserve their own vertical row instead of overlapping');
assert.match(stylesCss, /\.site-chat-msg-body\s*{[\s\S]*display: block;[\s\S]*flex: 0 1 auto;[\s\S]*min-width: 0;[\s\S]*overflow-wrap: anywhere;/, 'chat message body should wrap long markdown content inside the panel');
